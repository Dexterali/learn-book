...
int maxDepth(TreeNode *root)
{
    ...
    //更新我们的最大直径值
    //左右子树最大深度相加，即为当前树的最大直径，这个直径和整体进行比较。
...
int minDepth(TreeNode *root)
{
    ...
    //此处是关键，当前条件是叶子节点的判断条件，如果当前节点是叶子节点，那么当前节点的值就是 1
    if (root->left == nullptr && root->right == nullptr)
    {
        return 1;
    }

    // 这里有个细节需要注意一下，计算当前节点的最小深度的时候，因为需要的是访问的叶子节点
...
// 二叉树问题中，有一类和树的深度相关的问题，总结一套模板，遇到类似问题的时候，稍作改变，即可解决深度相关的一类问题。
// 题解可以看注释。
// 十三个二叉树问题
...
// 9. 路径总和 II https://leetcode-cn.com/problems/path-sum-ii/

// 题解一：比较好理解，但是 vector 数据的拷贝比较大
...
// 本题和 面试题 04.12. 求和路径 实际是一道题，只不过本题的测试用例要多一些，数据量要大一些
// 下面有个参数 sum 需要传递 long long int，或者可以改成相减的方式。
